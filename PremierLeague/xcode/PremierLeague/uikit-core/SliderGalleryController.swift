...
class SliderGalleryController: UIViewController, UIScrollViewDelegate{
    ...
    //自动滚动计时器
    let isEnableAutoScroll:Bool = false
    ...
    func configureAutoScrollTimer() {
        //设置一个定时器，每三秒钟滚动一次
        autoScrollTimer = Timer.scheduledTimer(timeInterval: TimeInterval(TimeLoop), target: self, selector: #selector(SliderGalleryController.letItScroll), userInfo: nil, repeats: true)
    }
